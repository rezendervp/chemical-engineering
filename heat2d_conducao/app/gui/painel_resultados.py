...
class AbaAnimacao(AbaCanvas):
    ...
    def carregar(self, frames, tempos, fps, mesh, cmap="turbo"):
        ...
        # cria a figura/eixos/colorbar UMA VEZ (não a cada quadro) -- recriar o
        # widget do canvas em toda atualização causa um "piscar" visível
        # (o widget aparece pequeno no canto e só depois é redimensionado).
        # Recriar o canvas continua necessário ao trocar de CASO (ghosting
        # entre simulações diferentes, ver mostrar_figura), mas dentro da
        # mesma animação só atualizamos os dados do gráfico já existente.
        self.canvas.get_tk_widget().destroy()
        plt.close(self.fig)
        self.fig = plt.figure(figsize=self.figsize)
        self.canvas = FigureCanvasTkAgg(self.fig, master=self._frame_canvas)
        ...
        self._ax.set_xlabel("x [m]"); self._ax.set_ylabel("y [m]"); self._ax.set_aspect("equal")
        cf = self._ax.contourf(self.mesh.X, self.mesh.Y, frames[0], levels=40,
                                cmap=self.cmap, vmin=self._vmin, vmax=self._vmax)
        self._colorbar = self.fig.colorbar(cf, ax=self._ax, label="Temperatura [°C]")
    ...
    def _desenhar_frame(self, idx):
        """Atualiza o quadro exibido SEM recriar o widget do canvas (evita o
        'piscar' de recriar a figura inteira a cada frame da animação)."""
        if not self.frames:
            return
        self._ax.clear()
        self._ax.set_xlabel("x [m]"); self._ax.set_ylabel("y [m]"); self._ax.set_aspect("equal")
