...
class JanelaPrincipal(ctk.CTk):
    ...
    def iniciar_simulacao(self):
        ...
        elif params["esquema"] == "implicito":
            self.painel_resultados.preparar_convergencia(
                self.painel_entrada.opt_solver.get(), modo="passo_tempo")
        else:
            self.painel_resultados.convergencia_sem_residuo(
                "Esquema explícito: não há sistema linear a resolver a cada passo,\n"
                "portanto não existe resíduo de solver para mostrar aqui.\n\n"
                "(A estabilidade do esquema explícito é garantida a priori pelo\n"
                "Δt crítico, mostrado no painel de entrada -- não por convergência iterativa.)")
